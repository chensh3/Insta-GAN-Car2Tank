...
class InstaGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_A_old = networks.define_D(opt.output_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.norm, use_sigmoid,
                                                opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_B_old = networks.define_D(opt.input_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.norm, use_sigmoid,
                                                opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_A = networks.Modelresnet50
            self.netD_A.to(self.gpu_ids[0])
            # self.netD_A = torch.nn.DataParallel(self.netD_A, self.gpu_ids)
            self.netD_B = networks.Modelresnet50
            self.netD_B.to(self.gpu_ids[0])
    ...
    @staticmethod
    def tank_fake(labels, tank=847):
        if labels.argmax == tank:
    ...
    def backward_G(self):
        ...
        # backward A
        if self.forward_A:
            self.loss_G_A = self.criterionGAN(self.tank_fake(self.netD_A(self.fake_B_mul[:, :-1, :, :])), True)
            self.loss_cyc_A = self.criterionCyc(self.rec_A_sng, self.real_A_sng) * lambda_A
            self.loss_idt_B = self.criterionIdt(self.netG_B(self.real_A_sng),
                                                self.real_A_sng.detach()) * lambda_A * lambda_idt
        ...
        # backward B
        if self.forward_B:
            self.loss_G_B = self.criterionGAN(self.tank_fake(self.netD_B(self.fake_A_mul[:, :-1, :, :])), True)
            self.loss_cyc_B = self.criterionCyc(self.rec_B_sng, self.real_B_sng) * lambda_B
            self.loss_idt_A = self.criterionIdt(self.netG_A(self.real_B_sng),
                                                self.real_B_sng.detach()) * lambda_B * lambda_idt
    ...
    def backward_D_basic(self, netD, real, fake):
        # Real
        pred_real = self.tank_fake(netD(real[:, :-1, :, :]))
        loss_D_real = self.criterionGAN(pred_real, True)
        # Fake
        pred_fake = self.tank_fake(netD(fake.detach()[:, :-1, :, :]))
